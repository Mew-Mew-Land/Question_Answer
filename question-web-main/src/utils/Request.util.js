...
//拦截器: 使用 axios 拦截器可以在请求发送前后进行操作。例如，这里在请求前启动一个加载动画，并在出错时关闭加载动画并显示错误信息。

// 定义常量表示 JSON 内容类型，通常用于 HTTP 请求的头部（Header）中指定内容格式。
const contentTypeJson = "application/json";

// 生产环境的基础 URL，用于真实用户环境的数据交互。
// /生产环境是软件发布后的运行环境，这是最终用户使用的环境。生产环境的服务器配置和数据是真实的，所有的功能都必须在此环境中完美运行。
...
const request = async (config) => {
  ...
  const {
    url, // 请求的 URL
    params, // 请求的参数
    showLoading = true, // 是否显示加载动画，默认为 true
    errorCallback, // 错误回调函数
    showError = true, // 是否显示错误消息，默认为 true
  } = config;
